...
class Library {
  /**
   * @param {any} books Books to initialize the Library with.
   */
  constructor(books) {
    this.books = [];
...
`
  ...
  bookCard.querySelector("button").addEventListener("click", () => bookLibrary.removeBookFromLibrary(book.id))
...
const updateUI = () => {
  ...
  const addElements = bookLibrary.books.filter(book => !displayedBooks.includes(book.id));
  const removeElements = displayedBooks.filter(elt => !bookLibrary.books.find((book) => book.id === elt));
...
booksForm.addEventListener("submit", (e) => {
  ...
  bookLibrary.addBookToLibrary(bookData);
